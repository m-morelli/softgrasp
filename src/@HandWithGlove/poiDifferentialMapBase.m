%HandWithGlove.POIDIFFERENTIALMAPBASE. Complete Hand Jacobian in World Coordinates
...
function [j, j_s_cell] = poiDifferentialMapBase(glv, q)

    % number of inputs must == 2
    narginchk(2, 2);

    % do the job
    [j, j_s_cell] = poiDifferentialMap(glv, q, @jacob0);

    % perform planar simplifications?
    if glv.isProblem2d,
        rp = glv.pReducedProb;
        j = rp.reduceDiffMotionsMapBaseTo2d(j);
