% GraspForceOptimizer.LLCS_VDVHV. Evaluate the Merit Function (V), its gradient (DV), and HV its Hessian matrix (HV) for GFO computations
...
function [v,dv,hv,s] = llcs_vdvhv(gfo, yo)
    ...
    rp = gfo.pReducedProb;
    Fe = gfo.wExt;
    E=gfo.controllableGraspForces;
    Gr=gfo.extCtForceTransmission;
    minF=gfo.minF;
    maxF=gfo.maxF;
    cs = gfo.contactStructure;
    cidx = 1:cs.nContactModels;
    cidx(gfo.excludeContacts) = [];
    %
    % optim options.
    de=gfo.de;
    wf=gfo.wf;
    ...
    [tcf, tcm] = cs.isolateSingleContributionsFrom(tc, rp, true);
    [M, N] = cs.isolateSingleContributionsFrom(E, rp, true);
    ...
    for i=cidx,
        ...
        Cni = rp.frameNormalLocal;

        % contrib of friction
        s_fr  = ai*fin + bi*min_ - Cni'*fic;
        ds_fr = ai*Mi'*fiv + bi*Ni'*miv - Mi'*Cni;
        hs_fr = (ai*Mi'*(eye(numel(fiv)*[1,1]) - fiv * fiv')*Mi) / fin;
        if s_fr >= - de,
        ...
        s_max  = fin - maxF;
        ds_max = Mi'*fiv;
        hs_max = (Mi'*(eye(numel(fiv)*[1,1]) - fiv * fiv')*Mi) / fin;
        if s_max >= - de,
