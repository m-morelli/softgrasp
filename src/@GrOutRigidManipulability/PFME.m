%GrOutRigidMobility.PFME. Compute the Passive Force Manipulability Ellipsoid for Rigid Grasps Without Synergies
...
function [V, D, tw, ttt] = PFME(grOutRM, ww_, wtt_)
    ...
    if ~(isempty(gmh) || all(all(gmh == 0,2))),
        gmt = llcs_projector_matrix(gmh, wtt)*gmt;
    end

    % setting up the eigenvalue problem of passive force manipulability
    num = blkdiag(gmw'*ww*gmw, gms'*ww*gms);
    den = blkdiag(gmt'*wtt*gmt, zeros(grOutRM.ns, grOutRM.ns));
    tw = [gmw, gms];
    ttt = [gmt, zeros(grOutRM.na, grOutRM.ns)];
    [V,D] = eig(num, den);