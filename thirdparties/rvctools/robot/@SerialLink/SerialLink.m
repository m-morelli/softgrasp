...
% MM, July 2013 -- begin --
% Modified by Matteo Morelli, July 2013
% - reintroduced the dh() method to get the DH matrix
%   [a, alpha, d, theta], that is needed by our legacy
%   code (pre-RTB9.X release)
% - fixed Robot constructor for 'double' inputs:
%   - modified DH params order
%   - added offset
%   => DH params order is now consistent with the one in the Link constructor
%      (i.e., [theta, d, a, alpha, sigma, offset])
% MM, July 2013 -- end --
    ...
    methods
        ...
        % S = R.char() is a string representation of the robot parameters.
            ...
            for j=1:length(robot)
                ...
                s = char(s, sprintf('%s (%d axis, %s, %s)', r.name, r.n, r.config, convention));
                ...
                s = strvcat(s, '+---+-----------+-----------+-----------+-----------+');
                if r.n > 0, s = strvcat(s, char(r.links, true)); end
                s = strvcat(s, '+---+-----------+-----------+-----------+-----------+');
        ...
        % MM, July 2013 -- begin --
        function dhMat = get.dh(obj)
            dhMat = zeros(obj.n,4);
            for i = 1:obj.n,
                dhMat(i,:) = obj.links(i).dh;
            end 
        end
        % MM, July 2013 -- end --
