...
%    end
    ...
    if robot.mdh == 0,
        while count > 0
            for p=1:np
                for r=rh',
                    animate_std( get(r, 'UserData'), tg(p,:), opt);
                    pause(opt.delay)
                end
            end
            count = count - 1;
        ...
        while count > 0
            for p=1:np
                for r=rh',
                    animate_mod( get(r, 'UserData'), tg(p,:), opt);
                    pause(opt.delay)
                end
            end
            count = count - 1;
...
function o = plot_options(robot, optin)
    ...
    if isempty(o.workspace),
        ...
        for i=1:robot.n,
            ...
            reach = reach + abs(L(i).a) + abs(L(i).d) + L(i).sigma;%*L(i).offset;
        end
        %o.workspace = [-reach reach -reach reach -reach reach];
        works = [-reach -reach -reach; reach reach reach] + kron(ones(2,1),transl(robot.base)');
...
function animate_std(robot, q, opt)
...
function animate_mod(robot, q, opt)
